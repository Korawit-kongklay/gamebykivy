...
class Game(Widget):
    ...
    def initialize_game(self):
        ...
        if self.ENABLE_PLAYER:
            self.player = Player(pos=(100 * Window.width / 1280, 0), health=self.initial_player_hp)
    ...
    def on_window_resize(self, window, width, height):
        ...
        # Clear and respawn stage (platforms are handled by Stage)
        ...
        if self.player:
            scale_x = width / 1280
            scale_y = height / 720
            self.player.pos = (100 * scale_x, 0)  # Scale initial position
            self.player.size = (80 * scale_x, 80 * scale_y)  # Scale player size

        # Respawn enemies with scaled sizes
        if self.ENABLE_ENEMIES:
    ...
    def _on_keyboard_down(self, keyboard, keycode, text, modifiers):
        ...
        if keycode[1] == 'spacebar':
            if self.can_jump(self.player):
                self.player.velocity_y = 6 * (Window.height / 720)
